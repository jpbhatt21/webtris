...
function initGame(
    ...
    if(!started)
        attachEventListener(setControls, setPaused,board);
}

let interLeft:any=null
let interRight:any=null
let interSoft:any=null
let odrop = false;  
function attachEventListener(setControls: any, setPaused: any,board:any) {
	window.addEventListener("keydown", (e) => {
		...
		if (!autoplay) {
			if (key === settings.moveLeft) {
                moveLeft(board)
				interLeft= interLeft || setInterval(() => {
                    moveLeft(board)
                }, lrSpeed);
			}
			if (key === settings.moveRight) {
				moveRight(board)
                interRight= interRight || setInterval(() => {
                    moveRight(board)
                }, lrSpeed);
			}
			if (key === settings.softDrop) {
				
                interSoft= interSoft || setInterval(() => {
                    softDrop(board)
                    odrop = true;
                },lrSpeed);
			}
...
let moveLeft:Function,moveRight:Function,softDrop:Function,rotateCW:Function;
export function startMainGameLoop({board,setBoard,setLcRect,setScore,setLinesCleared,setLevel,setActive,setGameOver,setNextShape,setCurShape,setPaused,setControls,setAutp,setHoldShape,setGhost}:any) {
    initGame(setLinesCleared,setScore,setLevel,setActive,setGhost,setNextShape,setLcRect,setCurShape,setHoldShape,setControls,setPaused,board);
