		...
		if (inter == null) {
			...
			function createNewPiece(
				...
				if (hldr) {
					...
					setScore(
						(prev) =>
							prev + [0, 40, 100, 300, 1200][lines] * (lv + 1)
					);
			...
			inter = setInterval(() => {
				...
				if (!move && autoplay) {
					changehold = automateAnalyzer(
						JSON.parse(JSON.stringify(board)),
						JSON.parse(JSON.stringify(act)),
						JSON.parse(JSON.stringify(shape)),
						JSON.parse(JSON.stringify(rot)),
						JSON.parse(JSON.stringify(nxt)),
						hld,
						hld > -1 && !held
					);
					if (!changehold) {
						[act, shape, rot] = automatic(
							JSON.parse(JSON.stringify(board)),
							JSON.parse(JSON.stringify(act)),
							JSON.parse(JSON.stringify(shape)),
							rot
		...
		<>
            ...
            <div className="w-full flex flex-col h-2/3 items-center justify-center">
				...
				<svg
					...
					{ghost.map((pos, ind) => (
						<rect
							className="duration-[25ms] brightness-75 ease-in-out"
							width={100}
							height={100}
							rx={10}
							x={5 + pos[1] * 105}
