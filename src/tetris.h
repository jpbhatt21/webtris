...
bool createNewPiece(
    ...
    int line = 0;
    for (int i = 0; i < 20; i++) {
        if (all_of(board[i].begin(), board[i].end(), [](bool x) { return x; })) {
...
pair<vector<vector<int>>, int> rotPiece(
...
pair<double, vector<vector<int>>> automate(
    vector<vector<bool>>& board,
    vector<vector<int>>& active,
    int shape,
    int rot
...
int& shape,
int& nxt,
vector<vector<int>>& active,vector<vector<vector<int>>> activePos
) 
 {
    // Initialize random
    held = false;
...
int activateSimulation(WeightSet weight) {
    ...
    // double duration = chrono::duration_cast<chrono::milliseconds>(endTime - startTime).count()/1000.0;
